...
def resetAll(strip, wait_ms=50):
    """Set all pixels to green one at a time"""
    colorWipe(strip, Color(0, 255, 0), wait_ms)


def setDemocrats(strip, electors, wait_ms=50):
    ...
    print("Num LEDS to change " + num_leds_to_change)
    ...
    print("Remainder " + remainder_level)
    if (PI):
        ...
        if remainder_level > 0:
            strip.setPixelColor(num_leds_to_change, Color(0, 0, math.floor(255 / (538 // LED_COUNT * (remainder_level + 1)))))
            strip.show()
            time.sleep(wait_ms/1000.0)


def setRepublicans(strip, electors, wait_ms=50):
    ...
    if (PI):
        for i in range(num_leds_to_change):
            ...
            time.sleep(wait_ms/1000.0)
        if remainder_level > 0:
            strip.setPixelColor(LED_COUNT - num_leds_to_change, Color(math.floor(255 / (538 // LED_COUNT) * (remainder_level + 1)), 0, 0))
            strip.show()
            time.sleep(wait_ms/1000.0)
    ...
    while True:
        ...
        try:
            ...
            if (dems,reps) != (dems_last,reps_last):
                ...
                setDemocrats(strip, dems)
                print("Rep electors " + reps)
                setRepublicans(strip, reps)
